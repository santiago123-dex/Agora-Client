...
function MarkdownContent({ text }: { text: string }) {
  const html = useMemo(() => {
    let result = text
      ...
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
      .replace(/```(\w*)\n?([\s\S]*?)```/g, '<pre class="rounded-lg bg-slate-100 p-2 my-1 overflow-x-auto text-xs dark:bg-slate-800"><code>$2</code></pre>')
      .replace(/`([^`]+)`/g, '<code class="rounded bg-slate-100 px-1 dark:bg-slate-800">$1</code>')
      .replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>")
      .replace(/\*([^*]+)\*/g, "<em>$1</em>")
      .replace(/^### (.+)$/gm, "<h4 class='text-sm font-semibold my-1'>$1</h4>")
      .replace(/^## (.+)$/gm, "<h3 class='text-base font-semibold my-1'>$1</h3>")
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: res.message, blocks: res.blocks }]);
    } catch {
      setMessages((prev) => [
        ...prev,
  ...
  return (
    ...
    >
      ...
      <div ref={listRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "flex-col items-start"}`}>
            <div
